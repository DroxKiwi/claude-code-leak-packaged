	...
	if (isOAuthNetworkDisabledForFork()) {
		return (
			<Dialog
				...
				onCancel={() =>
					onDone(
						'OAuth réseau désactivé — DROX_API_KEY + DROX_API_BASE_URL ou Ollama ; opt-in OAuth : DROX_OAUTH_NETWORK_ENABLED=1.',
					)
				}
			>
				<Text>
					Pas de navigateur OAuth ici : authentifiez-vous avec la clé API et la base URL (voir
					docs/GUIDE-REFONTE-DROX.md §2.2.1). Pour réactiver le flux cloud :
					DROX_OAUTH_NETWORK_ENABLED=1.
				</Text>
			</Dialog>
		)
	}
