...
import { getCloudBillingSettingsUrl } from '../../constants/product.js'
...
export const call: LocalJSXCommandCall = async (onDone, context, args) => {
	...
	if (gate.kind === 'not-enabled') {
		const billingUrl = getCloudBillingSettingsUrl()
		onDone(`Free ultrareviews used. Enable Extra Usage at ${billingUrl} to continue.`, {
			display: 'system',
		})
		return null
	}
	if (gate.kind === 'low-balance') {
		const billingUrl = getCloudBillingSettingsUrl()
		onDone(
			`Balance too low to launch ultrareview ($${gate.available.toFixed(2)} available, $10 minimum). Top up at ${billingUrl}`,
			{
				display: 'system',
			},
		)
