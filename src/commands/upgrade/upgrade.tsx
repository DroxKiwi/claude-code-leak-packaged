...
import { getClaudeAiUpgradeMaxUrl } from '../../constants/product.js'
...
export async function call(
	...
	if (isOAuthNetworkDisabledForFork()) {
		setTimeout(
			onDone,
			0,
			'/upgrade : réseau OAuth cloud désactivé pour ce fork. Voir docs/GUIDE-REFONTE-DROX.md §2 — DROX_OAUTH_NETWORK_ENABLED=1 pour le flux cloud, ou DROX_API_KEY + backend compatible.',
		)
		return null
	}
	try {
		...
		const url = getClaudeAiUpgradeMaxUrl()
		...
		setTimeout(
			onDone,
			0,
			`Failed to open browser. Please visit ${getClaudeAiUpgradeMaxUrl()} to upgrade.`,
		)
	}
	return null
}
