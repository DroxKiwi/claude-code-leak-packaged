...
import { UltraplanPollError, pollForApprovedExitPlanMode } from '../utils/ultraplan/ccrSession.js'
...
export function UltraplanChoiceDialog(_props: {
	plan: string
	sessionId: string
	taskId: string
	setMessages: (...args: never[]) => void
...
export function UltraplanLaunchDialog(_props: {
	onChoice: (choice: 'confirm' | 'cancel', opts?: { disconnectedBridge?: boolean }) => void
}): null {
	return null
}
		...
		try {
			...
			if (executionTarget === 'remote') {
				...
				enqueuePendingNotification({
					value: [
						`Ultraplan approved — executing in Claude Code in remote session. Follow along at: ${url}`,
						'',
...
export async function launchUltraplan(opts: {
	...
	if (!blurb && !seedPlan) {
		...
		return [
			...
			'(Opus). Runs in Claude Code in remote session. When the plan is ready,',
...
export default {
	type: 'local-jsx',
	name: 'ultraplan',
	description: `~10–30 min · Claude Code in remote session drafts an advanced plan you can edit and approve. See ${CCR_TERMS_URL}`,
	argumentHint: '<prompt>',
