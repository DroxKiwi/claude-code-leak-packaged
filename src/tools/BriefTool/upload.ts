 ...
 *
 * Base URL utilisateur via `DROX_API_BASE_URL` uniquement (pas de repli OAuth implicite).
 */
function getBridgeBaseUrl(): string {
	return getBridgeBaseUrlOverride() ?? getProcessEnvInferenceBaseUrl() ?? ''
}
...
export async function uploadBriefAttachment(
	...
	if (feature('BRIDGE_MODE')) {
		...
		if (!baseUrl) {
			debug('skip upload: DROX_API_BASE_URL / bridge base URL absente')
			return undefined
		}
		const url = `${baseUrl.replace(/\/$/, '')}/api/oauth/file_upload`
