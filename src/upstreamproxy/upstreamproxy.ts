...
export async function initUpstreamProxy(opts?: {
	...
	// CCR peut injecter `ccrBaseUrl` ; sinon uniquement `DROX_API_BASE_URL` —
	// aucune URL cloud implicite.
	const baseUrl = opts?.ccrBaseUrl ?? getProcessEnvInferenceBaseUrl() ?? ''
	if (!baseUrl) {
		logForDebugging('[upstreamproxy] pas de base URL (DROX_API_BASE_URL) ; proxy désactivé')
		return state
	}
