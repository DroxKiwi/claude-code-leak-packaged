...
import { isOAuthNetworkDisabledForFork, isRunningOnHomespace } from '../../utils/envUtils.js'
import { errorMessage } from '../../utils/errors.js'
import { logError } from '../../utils/log.js'
import { getAPIProvider } from '../../utils/model/providers.js'
import { getInitialSettings } from '../../utils/settings/settings.js'
import { jsonStringify } from '../../utils/slowOperations.js'
import { buildAPIProviderProperties, buildAccountProperties } from '../../utils/status.js'
...
export async function authStatus(opts: {
	...
	const hasApiKeyEnvVar = !!process.env.DROX_API_KEY && !isRunningOnHomespace()
	const oauthAccount = getOauthAccountInfo()
	const subscriptionType = getSubscriptionType()
	const using3P = isUsing3PServices()
	const loggedIn = hasToken || apiKeySource !== 'none' || hasApiKeyEnvVar || using3P

	// Determine auth method (fork Drox : clé via DROX_API_KEY / flux minimal uniquement)
	let authMethod = 'none'
	if (using3P) {
		authMethod = 'third_party'
	} else if (authTokenSource === 'DROX_API_KEY') {
		authMethod = 'api_key'
	} else if (authTokenSource !== 'none') {
		authMethod = 'oauth_token'
	} else if (apiKeySource === 'DROX_API_KEY' || hasApiKeyEnvVar) {
		authMethod = 'api_key'
	...
	if (opts.text) {
		...
		if (!hasAuthProperty && hasApiKeyEnvVar) {
			process.stdout.write('API key: DROX_API_KEY\n')
		}
		if (!loggedIn) {
		...
		const resolvedApiKeySource =
			apiKeySource !== 'none' ? apiKeySource : hasApiKeyEnvVar ? 'DROX_API_KEY' : null
...
export async function authLogout(): Promise<void> {
	...
	process.stdout.write(
		isOAuthNetworkDisabledForFork()
			? 'Déconnexion effectuée : identifiants et caches locaux effacés.\n'
