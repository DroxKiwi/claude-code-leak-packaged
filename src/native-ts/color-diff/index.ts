...
let cachedHljs: HLJSRuntime | null = null
function hljs(): HLJSRuntime {
	if (cachedHljs) return cachedHljs
	// eslint-disable-next-line @typescript-eslint/no-require-imports
	const mod = require('highlight.js')
	// highlight.js uses `export =` (CJS). Under bun/ESM the interop wraps it
	// in .default; under node CJS the module IS the API. Check at runtime.
	cachedHljs = ('default' in mod && mod.default ? mod.default : mod) as HLJSRuntime
	return cachedHljs
}
...
export class ColorDiff {
	...
	render(themeName: string, width: number, dim: boolean): string[] | null {
		...
		for (let i = 0; i < entries.length; i++) {
			...
			const tokens: Block[] =
				marker === '-' ? [[defaultStyle(theme), code]] : highlightLine(hlState, code, theme)
