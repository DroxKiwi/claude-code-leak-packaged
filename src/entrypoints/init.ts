...
// Fork: instrumentation is a stub (no MeterProvider); only Perfetto may run.
import { configureGlobalAgents } from '../utils/proxy.js'
import { isBetaTracingEnabled } from '../utils/telemetry/betaSessionTracing.js'
import type { MetricOptions, TelemetryAttributes } from '../utils/telemetry/telemetryTypes.js'
...
async function setMeterState(): Promise<void> {
	...
	if (meter) {
		...
		const createAttributedCounter = (name: string, options: MetricOptions): AttributedCounter => {
			...
			return {
				add(value: number, additionalAttributes: TelemetryAttributes = {}) {
