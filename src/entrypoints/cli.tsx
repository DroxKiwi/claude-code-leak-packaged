...
// Defines globalThis.MACRO (version from package.json) for Bun direct runs;
// production bundle still inlines MACRO.* via esbuild define in build-bundle.ts.
 ...
 * Heavy paths use dynamic imports; static imports above are limited (bundle, MACRO shim, env).
 */
async function main(): Promise<void> {
	const args = process.argv.slice(2)
	const isOutOfScopeCommand = (cmd: string | undefined): boolean =>
		Boolean(
			cmd &&
				[
					'remote-control',
	...
	// Terminal-local fork: block remote/cloud orchestration commands by default.
	if (isOutOfScopeCommand(args[0]) || args.includes('--bg') || args.includes('--background')) {
		console.error('This command is disabled in the terminal-local fork scope.')
		return
	}

	// Fast-path for --version/-v: no full CLI / main.tsx load (MACRO from shim or bundle define)
	if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
		// MACRO.VERSION: shim in dev / Bun src; esbuild define replaces in dist bundle
