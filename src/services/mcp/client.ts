...
function getMcpToolTimeoutMs(): number {
	return Number.parseInt(process.env.MCP_TOOL_TIMEOUT || '', 10) || DEFAULT_MCP_TOOL_TIMEOUT_MS
}
...
function getRemoteMcpServerConnectionBatchSize(): number {
	return Number.parseInt(process.env.MCP_REMOTE_SERVER_CONNECTION_BATCH_SIZE || '', 10) || 20
}
		...
		try {
			...
			if (serverRef.type === 'sse') {
				...
				if (isRemoteCloudMechanicsDisabledForFork()) {
					logMCPDebug(
						name,
						'claude.ai MCP proxy skipped (remote cloud mechanics disabled for fork)',
					)
					throw new Error(
						'Claude.ai MCP connectors are disabled for this build. On external builds set DROX_REMOTE_CLOUD_FEATURES_ENABLED=1; on ant builds unset CLAUDE_CODE_DISABLE_REMOTE_CLOUD.',
					)
				}
				...
				const { createLinkedTransportPair } = await import('./InProcessTransport.js')
				const cuServer = await createComputerUseMcpServerForCli()
				inProcessServer = cuServer
				const [clientTransport, serverTransport] = createLinkedTransportPair()
				await cuServer.connect(serverTransport)
				transport = clientTransport
