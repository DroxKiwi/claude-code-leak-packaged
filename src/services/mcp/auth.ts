...
import { XaaTokenExchangeError, performCrossAppAccess } from './xaa.js'
import {
	acquireIdpIdToken,
	clearIdpIdToken,
	discoverOidc,
	...
	try {
		...
		if (error instanceof OAuthError) {
			...
			if (error.errorCode === 'invalid_client' && error.message.includes('Client not found')) {
				...
				const serverKey = getServerKey(serverName, serverConfig)
				if (existingData.mcpOAuth?.[serverKey]) {
					existingData.mcpOAuth[serverKey].clientId = undefined
					existingData.mcpOAuth[serverKey].clientSecret = undefined
					storage.update(existingData)
				}
			}
		}
