 ...
 */
export function wrapFetchWithX402(innerFetch: typeof globalThis.fetch): typeof globalThis.fetch {
	const wrapped = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
		...
		if (!paymentRequiredHeader) {
			logForDebugging('[x402] Received 402 but no X-Payment-Required header present')
			return response
		}
		...
		// Handle the payment
		const result = handlePaymentRequired(paymentRequiredHeader, getX402SessionSpentUSD())
		...
		if (retryResponse.status === 402) {
			...
			logForDebugging(`[x402] Payment accepted, response status: ${retryResponse.status}`)
		}
	...
	return Object.assign(wrapped, {
		preconnect:
			'preconnect' in innerFetch && typeof innerFetch.preconnect === 'function'
				? innerFetch.preconnect.bind(innerFetch)
				: (): void => {},
...
export function addX402AxiosInterceptor(instance: AxiosInstance): void {
	instance.interceptors.response.use(
		...
		async (error) => {
			if (!error.response || error.response.status !== 402 || !isX402Enabled()) {
				throw error
			}
