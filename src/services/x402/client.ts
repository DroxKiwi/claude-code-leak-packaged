...
function tokenAmountToUSD(amount: string): number {
	return Number.parseInt(amount, 10) / 10 ** USDC_DECIMALS
}
 ...
 */
function signEIP712(domainSeparator: Buffer, structHash: Buffer, privateKeyHex: string): string {
	const { sign } = require('node:crypto') as typeof import('crypto')
...
export function createPayment(
	...
	const validAfter = '0'
	const validBefore = String(Math.floor(Date.now() / 1000) + requirement.maxTimeoutSeconds)
