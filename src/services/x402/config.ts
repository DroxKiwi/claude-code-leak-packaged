...
import { type PaymentNetwork, type X402WalletConfig, X402_DEFAULTS } from './types.js'
...
function toChecksumAddress(address: string): string {
	...
	for (let i = 0; i < addr.length; i++) {
		checksummed += Number.parseInt(hash[i], 16) >= 8 ? addr[i].toUpperCase() : addr[i]
	}
	return checksummed
}
...
function deriveAddress(privateKeyHex: string): string {
	const { createPublicKey, createPrivateKey } = require('node:crypto') as typeof import('crypto')
