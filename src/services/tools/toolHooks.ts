...
export async function* runPostToolUseHooks<Input extends AnyObject, Output>(
	...
	try {
		...
		for await (const result of executePostToolHooks(
			...
			try {
				const hookMessage = result.message
				// Check if we were aborted during hook execution
				// IMPORTANT: We emit a cancelled event per hook
				if (hookMessage?.type === 'attachment' && hookMessage.attachment.type === 'hook_cancelled') {
				...
				if (
					hookMessage &&
					!(hookMessage.type === 'attachment' && hookMessage.attachment.type === 'hook_blocking_error')
				) {
					yield {
						message: hookMessage as AttachmentMessage | ProgressMessage<HookProgress>,
					}
				}
