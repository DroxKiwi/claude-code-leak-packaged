 ...
 *
 * Nécessite une base URL utilisateur (`DROX_API_BASE_URL`) — pas d’hôte cloud implicite.
 */
...
/** Valeur d’en-tête wire « anthropic-version » (Messages / fichiers compatibles). */
const MESSAGES_API_VERSION_HEADER = '2023-06-01'

function getDefaultApiBaseUrl(): string | undefined {
	const fromEnv = getProcessEnvInferenceBaseUrl() || process.env.CLAUDE_CODE_API_BASE_URL?.trim()
...
export async function downloadFile(fileId: string, config: FilesApiConfig): Promise<Buffer> {
	...
	if (!baseUrl?.trim()) {
		throw new Error(
			'Files API : définissez DROX_API_BASE_URL (ou CLAUDE_CODE_API_BASE_URL) ou passez baseUrl dans la config.',
		)
	}
	...
	const headers = {
		...
		'anthropic-version': MESSAGES_API_VERSION_HEADER,
...
export async function uploadFile(
	...
	if (!baseUrl?.trim()) {
		return {
			path: relativePath,
			error: 'Files API : DROX_API_BASE_URL non défini — upload désactivé dans ce fork.',
			success: false,
		}
	}
...
export async function listFilesCreatedAfter(
	...
	const baseUrl = config.baseUrl || getDefaultApiBaseUrl()
	if (!baseUrl?.trim()) {
		logDebug('listFilesCreatedAfter: pas de base URL, retour vide')
		return []
	}
	const headers = {
		Authorization: `Bearer ${config.oauthToken}`,
