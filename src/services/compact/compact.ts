	...
	try {
		...
		if (preCompactDiscovered.size > 0) {
			boundaryMarker.compactMetadata = {
				...(boundaryMarker.compactMetadata ?? { trigger: isAutoCompact ? 'auto' : 'manual', preTokens: preCompactTokenCount ?? 0 }),
 ...
 * Direction 'later': summarizes messages after the index, keeps earlier ones.
 *   Prompt cache for kept (earlier) messages is preserved.
 * Direction 'earlier': summarizes messages before the index, keeps later ones.
...
export async function partialCompactConversation(
	...
	try {
		const messagesToSummarize =
			direction === 'earlier' ? allMessages.slice(0, pivotIndex) : allMessages.slice(pivotIndex)
		// 'earlier' must strip old compact boundaries/summaries: for 'earlier',
		...
		// 'later' keeps them: summary_B sits AFTER kept (backward scan still
		...
		// 'earlier' prefix hits cache directly; 'later' sends all (tail wouldn't cache).
		// PTL retry breaks the cache prefix but unblocks the user (CC-1180).
		let apiMessages = direction === 'earlier' ? messagesToSummarize : allMessages
		...
		if (preCompactDiscovered.size > 0) {
			boundaryMarker.compactMetadata = {
				...(boundaryMarker.compactMetadata ?? { trigger: 'manual', preTokens: preCompactTokenCount ?? 0 }),
				preCompactDiscoveredTools: [...preCompactDiscovered].sort(),
			}
		}
		...
		// 'later': prefix-preserving -> boundary; 'earlier': suffix -> last summary
		const anchorUuid =
			direction === 'earlier'
