...
export async function exchangeCodeForTokens(
	...
	if (isOAuthNetworkDisabledForFork()) {
		throw new Error(
			'OAuth réseau désactivé (Ollama, CLAUDE_CODE_DISABLE_OAUTH_NETWORK, ou build externe sans DROX_OAUTH_NETWORK_ENABLED=1).',
		)
	}
...
export async function refreshOAuthToken(
	...
	if (isOAuthNetworkDisabledForFork()) {
		throw new Error(
			'OAuth refresh désactivé (Ollama, CLAUDE_CODE_DISABLE_OAUTH_NETWORK, ou build externe sans DROX_OAUTH_NETWORK_ENABLED=1).',
		)
	}
	const requestBody = {
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
	...
	let populatedFromEnvOnly = false
	if (envAccountUuid && envUserEmail && envOrganizationUuid) {
		if (!getGlobalConfig().oauthAccount) {
			...
			populatedFromEnvOnly = true
		}
	}

	if (isOAuthNetworkDisabledForFork()) {
		return populatedFromEnvOnly
	}
