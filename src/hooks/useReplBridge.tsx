...
export function useReplBridge(
	...
	useEffect(() => {
		...
		if (feature('BRIDGE_MODE')) {
			if (isRemoteCloudMechanicsDisabledForFork()) return
					...
					async function handleInboundMessage(msg: SDKMessage): Promise<void> {
						try {
							...
							if (feature('KAIROS_GITHUB_WEBHOOKS')) {
								...
								const { sanitizeInboundWebhookContent } = require(
									'../bridge/webhookSanitizer.js',
								) as typeof import('../bridge/webhookSanitizer.js')
	...
	const sendBridgeResult = useCallback(() => {
		if (feature('BRIDGE_MODE') && !isRemoteCloudMechanicsDisabledForFork()) {
