...
export function extractPlanFromLog(log: SDKMessage[]): string | null {
	...
	for (let i = log.length - 1; i >= 0; i--) {
		...
		const fullText = extractTextContent((msg as SDKAssistantMessage).message.content, '\n')
...
function extractTodoListFromLog(log: SDKMessage[]): TodoList {
	...
	const input = (todoListMessage as SDKAssistantMessage).message.content.find(
	...
	const poll = async (): Promise<void> => {
		...
		try {
			...
			const reviewTimedOut =
				task.isRemoteReview && Date.now() - task.pollStartedAt > REMOTE_REVIEW_TIMEOUT_MS
