...
import { RemoteSessionProgress, formatReviewStageCounts } from './RemoteSessionProgress.js'
type Props = {
...
export function formatToolUseSummary(name: string, input: unknown): string {
	...
	if (name === EXIT_PLAN_MODE_V2_TOOL_NAME) {
		return 'Review the plan in Claude Code in remote session'
	}
...
function UltraplanSessionDetail(t0) {
	...
	if (confirmingStop) {
		...
		if ($[12] === Symbol.for('react.memo_cache_sentinel')) {
			t7 = <Text dimColor={true}>This will terminate the Claude Code in remote session session.</Text>
			$[12] = t7
		} else {
			t7 = $[12]
		}
	...
	if ($[47] === Symbol.for('react.memo_cache_sentinel')) {
		t19 = {
			label: 'Review in Claude Code in remote session',
			value: 'open' as const,
		}
		$[47] = t19
	} else {
...
function ReviewSessionDetail(t0) {
		...
		if ($[5] === Symbol.for('react.memo_cache_sentinel')) {
			t4 = (
				<Text dimColor={true}>
					...
					and any findings so far are discarded.
				</Text>
			)
			$[5] = t4
		} else {
	...
	if ($[11] !== completed || $[12] !== onKill || $[13] !== running) {
		t3 = completed
			? [
					{
						label: 'Open in Claude Code in remote session',
						value: 'open',
					},
					{
						label: 'Dismiss',
