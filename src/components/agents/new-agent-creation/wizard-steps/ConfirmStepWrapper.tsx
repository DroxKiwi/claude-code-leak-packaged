...
export function ConfirmStepWrapper({ tools, existingAgents, onComplete }: Props): ReactNode {
	const { data } = useWizard<AgentWizardData>()
		...
		async (openInEditor: boolean): Promise<void> => {
			...
			try {
				await saveAgentToFile(
					wizardData.location! as Parameters<typeof saveAgentToFile>[0],
				...
				if (openInEditor) {
					const filePath = getNewAgentFilePath({
						source: wizardData.location! as Parameters<typeof getNewAgentFilePath>[0]['source'],
				...
				const message = openInEditor
					? `Created agent: ${chalk.bold(wizardData.finalAgent.agentType)} and opened in editor. If you made edits, restart to load the latest version.`
