	...
	if (feature('SHOT_STATS') && stats.shotDistribution) {
		...
		if (total > 0) {
			const totalShots = Object.entries(dist).reduce(
				(s_0, [count, sessions]) => s_0 + Number.parseInt(count, 10) * sessions,
				0,
			)
			const bucket = (min: number, max?: number) =>
				Object.entries(dist)
					.filter(([k]) => {
						const n_0 = Number.parseInt(k, 10)
...
function renderOverviewToAnsi(stats: ClaudeCodeStats): string[] {
	...
	if (feature('SHOT_STATS') && stats.shotDistribution) {
		...
		if (totalWithShots > 0) {
			const totalShots = Object.entries(dist).reduce(
				(s, [count, sessions]) => s + Number.parseInt(count, 10) * sessions,
				0,
			)
