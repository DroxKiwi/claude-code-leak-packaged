...
import { type PastedContent, getGlobalConfig, saveGlobalConfig } from '../../utils/config.js'
...
import { MACOS_OPTION_SPECIAL_CHARS, isMacosOptionChar } from '../../utils/keyboardShortcuts.js'
...
function PromptInput({
	...
	useEffect(() => {
		if (feature('ULTRAPLAN') && ultraplanTriggers.length) {
			addNotification({
				...
				text: 'This prompt will launch an ultraplan session in Claude Code in remote session',
				priority: 'immediate',
				timeoutMs: 5000,
			})
		} else {
		...
		async (inputParam: string, isSubmittingSlashCommand = false) => {
			...
			if (isAgentSwarmsEnabled()) {
				...
				if (directMessage) {
					...
					if (result.error === 'no_team_context') {
	...
	const handleModelSelect = useCallback(
		(model: string | null, _effort: EffortLevel | undefined) => {
			...
			setAppState((prev) => {
				wasFastModeDisabled =
					isFastModeEnabled() && !isFastModeSupportedByModel(model) && !!prev.fastMode
				return {
					...prev,
					mainLoopModel: model,
