...
import { TMUX_COMMAND, getSwarmSocketName } from '../../utils/swarm/constants.js'
import {
			...
			if (idleTeammates.length > 0) {
				void Promise.all(
					...
					setSelectedIndex((prev) =>
						Math.max(0, Math.min(prev, teammateStatuses.length - idleTeammates.length - 1)),
					)
				})
			}
			return
	...
	if ($[1] !== teamName || $[2] !== teammate.agentId || $[3] !== teammate.name) {
		t2 = () => {
			...
			listTasks(teamName).then((allTasks) => {
				...
				setTeammateTasks(
					allTasks.filter(
						(task) => task.owner === teammate.agentId || task.owner === teammate.name,
					),
				)
			})
			return () => {
			...
			<Box flexDirection="column">
				...
				<Text>
					...
					{stringWidth(teammate.prompt) > 80 && !promptExpanded && (
						<Text dimColor={true}> (p to expand)</Text>
					)}
				</Text>
			</Box>
		)
