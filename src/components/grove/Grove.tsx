...
import { getCloudDataPrivacySettingsUrl } from '../../constants/product.js'
...
const DROX_PRIVACY_SETTINGS_URL = getCloudDataPrivacySettingsUrl()
	...
	if ($[0] === Symbol.for('react.memo_cache_sentinel')) {
		t0 = (
			<Text>
				...
				<Text bold={true}>October 8, 2025</Text>. You can accept the updated terms today.
			</Text>
		)
		$[0] = t0
	} else {
		...
		t4 = (
			<Box paddingLeft={1}>
				<Text>
					...
					{t3}
					<Text>
						— Allow the use of your chats and coding sessions to train and improve Anthropic AI
						models. Change anytime in your Privacy Settings (
						<Link url={DROX_PRIVACY_SETTINGS_URL} />
						).
					</Text>
				</Text>
			</Box>
			...
			<Box flexDirection="column">
				...
				<Box paddingLeft={1}>
					<Text>
						...
						<Text>
							— To help us improve our AI models and safety protections, we're extending data
							retention to 5 years.
						</Text>
					</Text>
				</Box>
			</Box>
		...
		t3 = (
			<Box flexDirection="column" gap={1}>
				...
				<Box flexDirection="column">
					...
					<Text>
						Turning ON the improve Claude setting extends data retention from 30 days to 5 years.
						Turning it OFF keeps the default 30-day data retention. Delete data anytime.
					</Text>
				</Box>
			</Box>
		)
...
export function PrivacySettingsDialog(t0) {
	...
	if ($[9] === Symbol.for('react.memo_cache_sentinel')) {
		t5 = (
			<Text>
				Review and manage your privacy settings at <Link url={DROX_PRIVACY_SETTINGS_URL} />
			</Text>
		)
		$[9] = t5
	} else {
