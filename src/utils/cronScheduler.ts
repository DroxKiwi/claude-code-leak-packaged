	...
	async function load(initial: boolean) {
		...
		if (missed.length > 0) {
			for (const t of missed) {
				...
				nextFireAt.set(t.id, Number.POSITIVE_INFINITY)
			}
	...
	function check() {
		...
		function process(t: CronTask, isSession: boolean) {
			...
			if (next === undefined) {
				...
				logForDebugging(
					`[ScheduledTasks] scheduled ${t.id} for ${next === Number.POSITIVE_INFINITY ? 'never' : new Date(next).toISOString()}`,
				)
			}
			...
			if (t.recurring && !aged) {
				...
				const newNext =
					jitteredNextCronRunMs(t.cron, now, t.id, jitterCfg) ?? Number.POSITIVE_INFINITY
