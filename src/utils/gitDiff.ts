...
export function parseGitNumstat(stdout: string): NumstatResult {
	...
	for (const line of lines) {
		...
		const fileAdded = isBinary ? 0 : Number.parseInt(addStr ?? '0', 10) || 0
		const fileRemoved = isBinary ? 0 : Number.parseInt(remStr ?? '0', 10) || 0
...
export function parseShortstat(stdout: string): GitDiffStats | null {
	...
	return {
		filesCount: Number.parseInt(match[1] ?? '0', 10),
