...
function computeSearchText(msg: RenderableMessage): string {
	...
	switch (msgAny.type) {
		case 'user': {
			const c = msgAny.message.content
			if (typeof c === 'string') {
				...
				for (const b of c) {
					if (b.type === 'text') {
						...
						parts.push(toolResultSearchText(msgAny.toolUseResult))
					}
				}
				raw = parts.join('\n')
			}
		...
		case 'attachment': {
			...
			if (msgAny.attachment.type === 'relevant_memories') {
				raw = ((msgAny.attachment.memories as Array<{ content?: string }>) ?? [])
					.map((m) => m.content ?? '')
					.join('\n')
			} else if (
				...
				msgAny.attachment.type === 'queued_command' &&
				msgAny.attachment.commandMode !== 'task-notification' &&
				!msgAny.attachment.isMeta
			) {
				const p = msgAny.attachment.prompt as string | Array<{ type?: string; text?: string }>
				raw =
					typeof p === 'string'
						? p
		...
		case 'collapsed_read_search': {
			...
			if (msgAny.relevantMemories) {
				raw = (msgAny.relevantMemories as Array<{ content?: string }>).map((m) => m.content ?? '').join('\n')
			}
			break
		}
		default:
