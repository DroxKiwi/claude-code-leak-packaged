import { APIConnectionError, APIError, AuthenticationError, NotFoundError } from '@anthropic-ai/sdk'
...
export async function validateModel(model: string): Promise<{ valid: boolean; error?: string }> {
	...
	// Check if it matches DROX_CUSTOM_MODEL_OPTION (pre-validated by the user)
	if (normalizedModel === process.env.DROX_CUSTOM_MODEL_OPTION) {
		return { valid: true }
	}
