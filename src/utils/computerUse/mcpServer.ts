 ...
 */
async function tryGetInstalledAppNames(): Promise<
	Array<{ bundleId: string; displayName: string; path: string }> | undefined
> {
	...
	return apps
		...
		.map((app) => ({
			...
			displayName: app.displayName ?? app.appName ?? app.bundleId,
			path: app.path ?? `${homedir()}/Applications/${app.displayName ?? app.bundleId}.app`,
		}))
}
...
> {
	...
	const tools = (buildComputerUseTools as unknown as (...args: unknown[]) => unknown[])(
