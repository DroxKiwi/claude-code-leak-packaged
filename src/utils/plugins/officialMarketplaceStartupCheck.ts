...
export async function checkAndInstallOfficialMarketplace(): Promise<OfficialMarketplaceCheckResult> {
	...
	if (isRemoteCloudMechanicsDisabledForFork()) {
		logForDebugging(
			'Official marketplace auto-install skipped: remote cloud mechanics disabled for fork (DROX_REMOTE_CLOUD_FEATURES_ENABLED)',
		)
		return {
			installed: false,
			skipped: true,
			reason: 'remote_cloud_disabled',
		}
	}
