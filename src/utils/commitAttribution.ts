import { type UUID, createHash, randomUUID } from 'node:crypto'
...
import { type ModelName, getCanonicalName } from './model/model.js'
	...
	try {
		...
		for (const line of lines) {
			...
			if (line.includes('file changed') || line.includes('files changed')) {
				...
				const insertions = insertMatch ? Number.parseInt(insertMatch[1]!, 10) : 0
				const deletions = deleteMatch ? Number.parseInt(deleteMatch[1]!, 10) : 0
