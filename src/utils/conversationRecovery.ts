...
import { type FileHistorySnapshot, copyFileHistoryForResume } from './fileHistory.js'
...
export function restoreSkillStateFromMessages(messages: Message[]): void {
	for (const message of messages) {
		...
		if (message.attachment.type === 'invoked_skills') {
			const skills = (message.attachment as { skills?: Array<{ name?: string; path?: string; content?: string }> }).skills ?? []
...
export async function loadConversationForResume(
	...
	try {
		...
		messages.push(...(hookMessages as Message[]))
