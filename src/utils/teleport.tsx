...
import { type TeleportRemoteResponse, deserializeMessages } from './conversationRecovery.js'
...
import { isEnvTruthy, isRemoteCloudMechanicsDisabledForFork } from './envUtils.js'
...
export async function teleportToRemote(options: {
	...
	try {
		if (isRemoteCloudMechanicsDisabledForFork()) {
			logForDebugging(
				'[teleportToRemote] skipped: remote cloud mechanics disabled (external fork: set DROX_REMOTE_CLOUD_FEATURES_ENABLED=1; ant: unset CLAUDE_CODE_DISABLE_REMOTE_CLOUD)',
			)
			return null
		}
