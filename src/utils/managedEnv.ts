...
import { SAFE_ENV_VARS, isProviderManagedEnvVar } from './managedEnvConstants.js'
...
/**
 * `claude ssh` remote: DROX_UNIX_SOCKET routes auth through a -R forwarded
...
function withoutSSHTunnelVars(env: Record<string, string> | undefined): Record<string, string> {
	if (!env || !process.env.DROX_UNIX_SOCKET) return env || {}
	const {
		DROX_UNIX_SOCKET: _1,
		DROX_API_BASE_URL: _2,
		DROX_API_KEY: _3,
		DROX_AUTH_TOKEN: _4,
 ...
 * traffic (e.g., DROX_API_BASE_URL) to an attacker-controlled server.
 */
 ...
 * DROX_API_BASE_URL take effect during first-run/onboarding.
 *
 * For trusted sources (user settings, managed settings, CLI flags), ALL env vars
 * are applied — including ones like DROX_API_BASE_URL that would be dangerous
...
export function applySafeConfigEnvironmentVariables(): void {
	...
	// DROX_API_BASE_URL — both settable via settings.env.
