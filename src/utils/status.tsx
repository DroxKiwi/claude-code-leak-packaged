...
export function buildAPIProviderProperties(): Property[] {
	...
	if (apiProvider === 'firstParty') {
		const anthropicBaseUrl = process.env.DROX_API_BASE_URL
		if (anthropicBaseUrl) {
			properties.push({
		...
		const gcpProject = process.env.DROX_VERTEX_PROJECT_ID
		if (gcpProject) {
			properties.push({
				label: 'GCP project',
				value: gcpProject,
		...
		const foundryBaseUrl = process.env.DROX_FOUNDRY_BASE_URL
		if (foundryBaseUrl) {
			properties.push({
		...
		const foundryResource = process.env.DROX_FOUNDRY_RESOURCE
		if (foundryResource) {
			properties.push({
