...
/**
 * Checks if the user has synced their GitHub credentials via /remote-setup
...
export async function checkGithubTokenSynced(): Promise<boolean> {
	try {
		...
		logForDebugging('Checking if GitHub token is synced via remote-setup')
 ...
 * 2. GitHub token synced via /remote-setup
