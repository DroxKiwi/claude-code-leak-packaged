...
function asUuid(value: string | undefined): UUID {
	const uuidPattern =
		/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i
	return value && uuidPattern.test(value) ? (value as UUID) : randomUUID()
}

export function toInternalMessages(messages: readonly DeepImmutable<SDKMessage>[]): Message[] {
	return (messages as unknown as readonly SDKMessage[]).flatMap((message) => {
		switch (message.type) {
			case 'assistant':
				return [
					{
