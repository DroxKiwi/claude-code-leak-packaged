...
import { MAX_MEMORY_CHARACTER_COUNT, type MemoryFileInfo, getLargeMemoryFiles } from './claudemd.js'
import type { getGlobalConfig } from './config.js'
import { getCwd } from './cwd.js'
import { formatNumber } from './format.js'
import { getTerminalIdeType, isSupportedJetBrainsTerminal, toIDEDisplayName } from './ide.js'
...
const claudeAiSubscriberExternalTokenNotice: StatusNoticeDefinition = {
	...
	isActive: () => {
		...
		const authSource = String(authTokenInfo.source)
		return (
			isClaudeAISubscriber() &&
			(authSource === 'DROX_AUTH_TOKEN' || authSource === 'apiKeyHelper')
		)
	},
	render: () => {
		const authTokenInfo = getAuthTokenSource()
		return (
			<Box flexDirection="row" marginTop={1}>
				<Text color="warning">{figures.warning}</Text>
				<Text color="warning">
					Auth conflict: Using {authTokenInfo.source} instead of Claude account subscription token.
					Either unset {authTokenInfo.source}, or run `claude /logout`.
				</Text>
			</Box>
		)
	},
...
const apiKeyConflictNotice: StatusNoticeDefinition = {
	...
	isActive: () => {
		...
		const source = String(apiKeySource)
		return (
			!!getApiKeyFromConfigOrMacOSKeychain() &&
			(source === 'DROX_API_KEY' || source === 'apiKeyHelper')
		)
	},
	render: () => {
		const { source: apiKeySource } = getAnthropicApiKeyWithSource({
...
const bothAuthMethodsNotice: StatusNoticeDefinition = {
	...
	isActive: () => {
		...
		const source = String(apiKeySource)
		const authSource = String(authTokenInfo.source)
		return source !== 'none' && authSource !== 'none' && !(source === 'apiKeyHelper' && authSource === 'apiKeyHelper')
	},
	render: () => {
		const { source: apiKeySource } = getAnthropicApiKeyWithSource({
			skipRetrievingKeyFromApiKeyHelper: true,
		...
		return (
			<Box flexDirection="column" marginTop={1}>
				...
				<Box flexDirection="column" marginLeft={3}>
					<Text color="warning">
						· Trying to use{' '}
						{authSource === 'claude.ai' ? 'claude.ai' : authSource}?{' '}
						{apiKeySource === 'DROX_API_KEY'
							? 'Unset the DROX_API_KEY environment variable, or claude /logout then say "No" to the API key approval before login.'
					...
					<Text color="warning">
						...
						{authSource === 'claude.ai'
							...
							: `Unset the ${authSource} environment variable.`}
					</Text>
				</Box>
			</Box>
		)
