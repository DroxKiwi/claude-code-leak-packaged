...
/** Aligné sur les hunks `diff` / contrats outils (oldLines / newLines requis côté types). */
...
/** Les hunks renvoyés par `diff` peuvent omettre oldLines/newLines ; on les normalise. */
function normalizeDiffHunk(hunk: {
	oldStart: number
	newStart: number
	lines: string[]
	oldLines?: number
...
export function getPatchFromContents({
	...
	const patchResult = result as unknown as
		| {
				hunks?: Array<{
					oldStart: number
					newStart: number
		...
		| undefined
	if (!patchResult?.hunks) {
		return []
	}
	return patchResult.hunks.map((hunk) =>
		normalizeDiffHunk({
			...hunk,
