...
import { type TLSConfig, getMTLSAgent, getMTLSConfig, getTLSFetchOptions } from './mtls.js'
 ...
 *
 * @param opts.forAnthropicAPI - Enables DROX_UNIX_SOCKET tunneling. This
...
export function getProxyFetchOptions(opts?: { forAnthropicAPI?: boolean }): {
	...
	// DROX_UNIX_SOCKET tunnels through the `claude ssh` auth proxy, which
	...
	if (opts?.forAnthropicAPI) {
		const unixSocket = process.env.DROX_UNIX_SOCKET
