...
import { type PaneBackendType, isPaneBackend } from './swarm/backends/types.js'
...
export function getTeammateStatuses(teamName: string): TeammateStatus[] {
	...
	for (const member of teamFile.members) {
		...
		statuses.push({
			...
			backendType:
				member.backendType && isPaneBackend(member.backendType) ? member.backendType : undefined,
			mode: member.mode,
		})
	}
