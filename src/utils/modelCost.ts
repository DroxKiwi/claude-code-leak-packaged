...
function tokensToUSDCost(modelCosts: ModelCosts, usage: Usage): number {
	...
	return (
		...
		(u.server_tool_use?.web_search_requests ?? 0) * modelCosts.webSearchRequests
	)
}
...
export function getModelCosts(model: string, usage: Usage): ModelCosts {
	...
	if (!costs) {
		...
		return (
			MODEL_COSTS[getCanonicalName(getDefaultMainLoopModelSetting())] ?? DEFAULT_UNKNOWN_MODEL_COST
		)
	}
	return costs
}
