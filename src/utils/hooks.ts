...
import { emitHookResponse, emitHookStarted, startHookProgressInterval } from './hooks/hookEvents.js'
import {
		...
		try {
			...
			if (hook.type === 'prompt') {
				...
				if (promptResult.message?.type === 'attachment') {
					...
					if (
						att &&
						(att.type === 'hook_success' || att.type === 'hook_non_blocking_error')
					) {
...
export async function executeStopFailureHooks(
	...
	const error = (lastMessage.error ?? 'unknown') as StopFailureHookInput['error']
...
export async function executeStatusLineCommand(
	...
	signal?: AbortSignal,
	timeoutMs = 5000, // Short timeout for status line
	logResult = false,
...
export async function executeFileSuggestionCommand(
	...
	signal?: AbortSignal,
	timeoutMs = 5000, // Short timeout for typeahead suggestions
): Promise<string[]> {
